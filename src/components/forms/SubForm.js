...
import { add } from '@/pages/Template/service';

const SubForm = ({optionColumns,type,singular,handleSubVisible,setParentValue,reloadList}) => {

    const handleAdd = async fields => {
        const hide = message.loading('Adding');
      
        try {
          const v = await add({ ...fields,type:type });
          await reloadList()    
          setParentValue(v._id)
          hide();
