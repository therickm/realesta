...
import { Input, Button, Table, Row, Col, Statistic, Badge, Icon, Space, Spin, Divider, Drawer, PageHeader } from 'antd';
import { map, } from "lodash";
import ReactHTMLTableToExcel from "react-html-table-to-excel";
import { processData } from '@/utils/utils';
import { CommentOutlined, EyeTwoTone } from '@ant-design/icons';
...
const Collection = ({ category }) => {
    ...
    const [ViewClientDrawer, setViewClientDrawer] = useState(false)
    ...
    return (
        <div>
            ...
            <Table pagination={{ pageSizeOptions: ["20", "50", "100", "500"], showSizeChanger: true, showQuickJumper: true, position: "bottom" }} ref={tableRef} loading={loadingData} dataSource={filteredData} columns={[...columns, ...actions]} size="small" />
            <Drawer
                visible={ViewClientDrawer}
                onClose={() => setViewClientDrawer(false)}
                width={840}
                closable={false}
            >
                <ClientDetails {...selectedClient} closeDrawer={() => { setSelectedClient({}), setViewClientDrawer(false) }} />
