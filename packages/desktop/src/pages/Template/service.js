...
export async function getTenantAndUnit(occupancyId) {
  const occupancy = await new DB('occupations').getDocument(occupancyId)
  const tenant = await new DB('tenants').getDocument(occupancy.tenant)
  const unit = await new DB('units').getDocument(occupancy.unit)
