...
export const processData = async (category, date = null) => {
  ...
  const TheData = ddata[0] && ddata[0].map(
    (doc, index) => {
      ...
      let rowToPush = {
        ...unit,
        ...tenant,
        landlord: `${landlord.sur_name} ${landlord.first_name}${landlord.middle_name ? " " + landlord.middle_name : ''}`,
      ...
      if (category === 'Rent') {
        rowToPush.months = numeral((totalInvoices - totalReceipts) / unit.rent).format('0.00')
